    ...
    grid.innerHTML = data.map(item => `
        <div class="card-anim group cursor-pointer p-6 rounded-3xl" onclick="openItem(${item.id})">
            <div class="aspect-[4/5] bg-stone-100 rounded-2xl mb-8 relative overflow-hidden flex items-center justify-center">
                ...
                ${item.author ? `<div class="absolute top-6 left-6 bg-stone-900 text-white px-3 py-1 rounded-full shadow-sm text-[8px] font-black tracking-widest uppercase">by ${escapeHtml(item.author)}</div>` : ''}
        ...
        content.innerHTML = `
            <div class="grid md:grid-cols-5 h-full">
                <div class="md:col-span-2 bg-stone-50 p-12 lg:p-20 border-r border-stone-100">
                    ${item.author ? `<p class="text-[10px] uppercase font-bold tracking-widest text-stone-400 mb-4">Shared by ${escapeHtml(item.author)}</p>` : ''}
                    <h2 class="text-6xl serif mb-6">${escapeHtml(item.title)}</h2>
                    ...
                    ${isOwner ? `<button onclick="deleteRecipe(${item.id})" class="mt-10 bg-red-600 hover:bg-red-700 text-white text-[10px] font-bold uppercase tracking-widest px-8 py-4 rounded-full transition">Delete this recipe</button>` : ''}
                </div>
...
async function submitRecipe(event) {
    ...
    try {
        ...
        const data = await res.json();
        if (!res.ok) return showShareError(data.error || 'Could not publish your recipe.');
